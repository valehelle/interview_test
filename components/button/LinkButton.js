...
const Button = ({ children, negative = false, type = 'docked', ...props }) => {
    const textSize = type == 'medium' ? 12 : 14
    const height = getHeight(type)
    const padding = children.length < 8 ? 38 : 16
    const width = type == 'docked' ? 200 : 'inherit'
    return (
        <StyledButton disableElevation height={height} width={width} textSize={textSize} negative={negative} padding={padding} color='primary' theme={customTheme} {...props}>{children}</StyledButton>
    )
}
